...
#' @param region Monarchy to use (UK, Scotland)
...
regnalYear <- function(date, region="UK", system="UK", abbr=FALSE) {
  ...
  if (region=="Scotland") {
    return(regnalYearScotland(date, system=system, abbr=abbr))
  }
}
...
regnalYearScotland <- function(date, system, abbr) {
  if (system=="UK") {
    if (date < "1752-09-02") {
      system <- "Julian"
    } else {
  ...
  if (system=="Gregorian") {
    row <- max(which(date >= ScotlandRegnal$GregorianStart))
    return(regnalYearFormat(date, ScotlandRegnal[row,], system=system, abbr=abbr))
  }
  if (system=="Julian") {
    row <- max(which(date >= ScotlandRegnal$JulianStart))
    return(regnalYearFormat(date, ScotlandRegnal[row,], system=system, abbr=abbr))
  }
}
...
regnalYearFormat <- function(date, data, system, abbr) {
  ...
  if (is.na(data$RegnalYear) | data$RegnalYear == "") {
    if (m>mr | (m==mr & d>= dr)) {
      ry <- y-yr+1
    } else {
      ry <- y-yr
